...
class PublicationController {
    ...
    static async registerAuthor(c) {
        ...
        const validation = publication_schema_1.authorRegistrationSchema.safeParse(body);
        if (!validation.success) {
            return c.json({ error: validation.error.flatten() }, 400);
        }
        try {
            const result = await publication_service_1.PublicationService.registerAuthor(user.id, validation.data);
    ...
    static async registerNewAuthor(c) {
        const body = await c.req.json();
        const validation = publication_schema_1.newAuthorRegistrationSchema.safeParse(body);
        if (!validation.success) {
            return c.json({ error: validation.error.flatten() }, 400);
        }
        try {
            const user = await publication_service_1.PublicationService.registerNewAuthor(validation.data);
            // Auto login
    ...
    static async updateAuthorProfile(c) {
        ...
        try {
            await publication_service_1.PublicationService.updateAuthorProfile(user.id, body);
    ...
    static async createArticle(c) {
        const user = c.get('user');
        const body = await c.req.json();
        const validation = publication_schema_1.createArticleSchema.safeParse(body);
        if (!validation.success) {
            return c.json({ error: validation.error.flatten() }, 400);
        }
        try {
            const article = await publication_service_1.PublicationService.createArticle(validation.data, user.id);
    ...
    static async updateArticle(c) {
        ...
        const user = c.get('user');
        const body = await c.req.json();
        const validation = publication_schema_1.updateArticleSchema.safeParse(body);
        if (!validation.success) {
            return c.json({ error: validation.error.flatten() }, 400);
        }
        try {
            const article = await publication_service_1.PublicationService.updateArticle(id, user.id, validation.data);
    ...
    static async approveArticle(c) {
        ...
        const user = c.get('user');
        try {
            const article = await publication_service_1.PublicationService.approveArticle(id, user.id);
    ...
    static async rejectArticle(c) {
        const id = Number(c.req.param('id'));
        const body = await c.req.json();
        const validation = publication_schema_1.approvalSchema.safeParse(body);
        if (!validation.success) {
            return c.json({ error: validation.error.flatten() }, 400);
        }
        try {
            const article = await publication_service_1.PublicationService.rejectArticle(id, validation.data.rejectionReason || 'No reason provided');
            // TODO: Send notification
    ...
    static async getArticles(c) {
        ...
        const validation = publication_schema_1.publicationFilterSchema.safeParse(query);
        if (!validation.success) {
            return c.json({ error: validation.error.flatten() }, 400);
        }
        try {
            ...
            const result = await publication_service_1.PublicationService.getArticles(filters);
    ...
    static async getAdminArticles(c) {
        ...
        try {
            ...
            const result = await publication_service_1.PublicationService.getArticles(validation.data);
            return c.json(result);
        }
        catch (e) {
    ...
    static async getArticleBySlug(c) {
        ...
        try {
            const article = await publication_service_1.PublicationService.getArticleBySlug(slug);
            if (!article)
    ...
    static async getArticleById(c) {
        const id = Number(c.req.param('id'));
        try {
            const article = await publication_service_1.PublicationService.getArticleById(id);
            if (!article)
                return c.json({ error: 'Artikel tidak ditemukan' }, 404);
            return c.json({ data: article });
        }
    ...
    static async deleteArticle(c) {
        const id = Number(c.req.param('id'));
        try {
            await publication_service_1.PublicationService.deleteArticle(id);
    ...
    static async createCategory(c) {
        const body = await c.req.json();
        const validation = publication_schema_1.createCategorySchema.safeParse(body);
        if (!validation.success) {
            return c.json({ error: validation.error.flatten() }, 400);
        }
        try {
            const category = await publication_service_1.PublicationService.createCategory(validation.data);
    ...
    static async getCategories(c) {
        ...
        try {
            const categories = await publication_service_1.PublicationService.getCategories(type);
    ...
    static async getAdminCategories(c) {
        try {
            const categories = await publication_service_1.PublicationService.getAdminCategories();
            return c.json({ data: categories });
        }
        catch (e) {
            return c.json({ error: 'Gagal mengambil kategori' }, 500);
    ...
    static async updateCategory(c) {
        const id = Number(c.req.param('id'));
        const body = await c.req.json();
        const validation = publication_schema_1.updateCategorySchema.safeParse(body);
        if (!validation.success) {
            return c.json({ error: validation.error.flatten() }, 400);
        }
        try {
            const category = await publication_service_1.PublicationService.updateCategory(id, validation.data);
    ...
    static async deleteCategory(c) {
        const id = Number(c.req.param('id'));
        try {
            await publication_service_1.PublicationService.deleteCategory(id);
    ...
    static async createVolume(c) {
        const body = await c.req.json();
        const validation = publication_schema_1.createVolumeSchema.safeParse(body);
        if (!validation.success) {
            return c.json({ error: validation.error.flatten() }, 400);
        }
        try {
            const volume = await publication_service_1.PublicationService.createVolume(validation.data);
    ...
    static async getVolumes(c) {
        try {
            const volumes = await publication_service_1.PublicationService.getVolumes();
    ...
    static async deleteVolume(c) {
        const id = Number(c.req.param('id'));
        try {
            await publication_service_1.PublicationService.deleteVolume(id);
    ...
    static async verifyAuthor(c) {
        ...
        try {
            const user = await publication_service_1.PublicationService.verifyAuthor(id, status, reason);
            let message = '';
            if (status === 'approved')
    ...
    static async getPendingAuthors(c) {
        try {
            ...
            const authors = await publication_service_1.PublicationService.getAdminAuthors();
    ...
    static async getDashboardStats(c) {
        try {
            const stats = await publication_service_1.PublicationService.getDashboardStats();
    ...
    static async getAuthorStats(c) {
        const user = c.get('user');
        try {
            const stats = await publication_service_1.PublicationService.getAuthorStats(user.id);
            return c.json({ data: stats });
        }
        catch (e) {
    ...
    static async createCollaboration(c) {
        const user = c.get('user');
        const body = await c.req.json();
        const validation = publication_schema_1.createCollaborationSchema.safeParse(body);
        if (!validation.success) {
            return c.json({ error: validation.error.flatten() }, 400);
        }
        try {
            const result = await publication_service_1.PublicationService.createCollaboration(user.id, validation.data);
    ...
    static async getCollaborations(c) {
        const user = c.get('user');
        try {
            const data = await publication_service_1.PublicationService.getCollaborations(user.id);
            return c.json({ data });
        }
        catch (e) {
    ...
    static async getCollaborationById(c) {
        ...
        try {
            const data = await publication_service_1.PublicationService.getCollaborationById(id, user.id);
            return c.json({ data });
        }
        catch (e) {
    ...
    static async updateCollaboration(c) {
        ...
        const id = Number(c.req.param('id'));
        const body = await c.req.json();
        const validation = publication_schema_1.updateCollaborationSchema.safeParse(body);
        if (!validation.success) {
            return c.json({ error: validation.error.flatten() }, 400);
        }
        try {
            const data = await publication_service_1.PublicationService.updateCollaboration(id, user.id, validation.data);
    ...
    static async deleteCollaboration(c) {
        ...
        try {
            const result = await publication_service_1.PublicationService.deleteCollaboration(id, user.id);
    ...
    static async addCollaborationMember(c) {
        ...
        const id = Number(c.req.param('id'));
        const body = await c.req.json();
        const validation = publication_schema_1.addCollaborationMemberSchema.safeParse(body);
        if (!validation.success) {
            return c.json({ error: validation.error.flatten() }, 400);
        }
        try {
            const result = await publication_service_1.PublicationService.addCollaborationMember(id, user.id, validation.data);
    ...
    static async inviteCollaborator(c) {
        ...
        const id = Number(c.req.param('id'));
        const body = await c.req.json();
        const validation = publication_schema_1.inviteCollaborationSchema.safeParse(body);
        if (!validation.success) {
            return c.json({ error: validation.error.flatten() }, 400);
        }
        try {
            const result = await publication_service_1.PublicationService.inviteCollaborator(id, user.id, validation.data);
    ...
    static async searchUsersForInvite(c) {
        ...
        try {
            const user = c.get('user');
            const list = await publication_service_1.PublicationService.searchUsersForInvite(q, 10, user.id);
    ...
    static async respondInvite(c) {
        ...
        const body = await c.req.json();
        const validation = publication_schema_1.respondInviteSchema.safeParse(body);
        if (!validation.success) {
            return c.json({ error: validation.error.flatten() }, 400);
        }
        try {
            const res = await publication_service_1.PublicationService.respondInvite(id, inviteId, user.id, validation.data);
    ...
    static async removeCollaborationMember(c) {
        ...
        try {
            await publication_service_1.PublicationService.removeCollaborationMember(id, user.id, memberId);
