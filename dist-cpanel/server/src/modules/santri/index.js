...
const findSantriByUser = async (userRecord) => {
    let record = await db_1.db.query.santri.findFirst({
        where: (0, drizzle_orm_1.eq)(schema_1.santri.email, userRecord.email),
    });
    if (!record && userRecord.phone) {
        record = await db_1.db.query.santri.findFirst({
            where: (0, drizzle_orm_1.eq)(schema_1.santri.noHp, userRecord.phone),
        });
    }
    return record;
};
app.get('/dashboard', async (c) => {
    ...
    const announcements = await db_1.db.query.blogAnnouncements.findMany({
        orderBy: [(0, drizzle_orm_1.desc)(schema_1.blogAnnouncements.createdAt)],
        limit: 5,
    });
    let step = 0;
...
app.get('/registration-status', async (c) => {
    ...
    try {
        const payment = await db_1.db.query.payments.findFirst({
            where: (0, drizzle_orm_1.eq)(schema_1.payments.santriId, existingSantri.id)
        });
        if (payment) {
...
app.post('/registration', async (c) => {
    ...
    if (existingSantri) {
        ...
        await db_1.db.update(schema_1.santri).set(updateData).where((0, drizzle_orm_1.eq)(schema_1.santri.id, existingSantri.id));
    ...
    const duplicateNisn = await db_1.db.query.santri.findFirst({
        where: (0, drizzle_orm_1.eq)(schema_1.santri.nisn, nisnRaw),
    });
    if (duplicateNisn) {
    ...
    try {
        await db_1.db.insert(schema_1.santri).values(data);
        if (newStatus === 'submitted') {
            await db_1.db.insert(schema_1.notifications).values({
                userId: currentUser.id,
                title: 'Pendaftaran Terkirim',
...
app.get('/notifications', async (c) => {
    const user = c.get('user');
    const userNotifications = await db_1.db.query.notifications.findMany({
        where: (0, drizzle_orm_1.eq)(schema_1.notifications.userId, user.id),
