import { db } from '../../db';
import { publicationArticles, publicationCategories, publicationVolumes, users, publicationProfiles, publicationCollaborations, publicationCollaborationMembers, publicationCollaborationInvites, notifications, publicationArticleAudits } from '../../db/schema';
...
export class PublicationService {
    ...
    static async registerNewAuthor(data) {
        try {
            ...
            const existingUser = await db.query.users.findFirst({
                where: or(ilike(users.username, data.username), ilike(users.email, data.email))
            });
            if (existingUser) {
    ...
    static async createArticle(data, authorId) {
        ...
        if (data.collaborationId) {
            const isMember = await db.query.publicationCollaborationMembers.findFirst({
                where: and(eq(publicationCollaborationMembers.collaborationId, data.collaborationId), eq(publicationCollaborationMembers.userId, authorId))
            });
            if (!isMember) {
    ...
    static async updateArticle(id, userId, data) {
        ...
        if (!isAuthorized && article.collaborationId) {
            const membership = await db.query.publicationCollaborationMembers.findFirst({
                where: and(eq(publicationCollaborationMembers.collaborationId, article.collaborationId), eq(publicationCollaborationMembers.userId, userId))
            });
        ...
        if (data.collaborationId && data.collaborationId !== article.collaborationId) {
            const isMember = await db.query.publicationCollaborationMembers.findFirst({
                where: and(eq(publicationCollaborationMembers.collaborationId, data.collaborationId), eq(publicationCollaborationMembers.userId, userId))
            });
            if (!isMember) {
    ...
    static async getArticles(filters) {
        const conditions = [];
        if (filters.search) {
            conditions.push(or(ilike(publicationArticles.title, `%${filters.search}%`), ilike(publicationArticles.content, `%${filters.search}%`)));
        }
        if (filters.status) {
    ...
    static async getAdminAuthors() {
        return await db.query.users.findMany({
            where: or(eq(users.publicationRole, 'author'), eq(users.isPublicationRegistered, true)),
            orderBy: [
    ...
    static async getCollaborationById(id, userId) {
        const membership = await db.query.publicationCollaborationMembers.findFirst({
            where: and(eq(publicationCollaborationMembers.collaborationId, id), eq(publicationCollaborationMembers.userId, userId))
        });
        if (!membership) {
    ...
    static async addCollaborationMember(collaborationId, requesterId, data) {
        ...
        const requesterMembership = await db.query.publicationCollaborationMembers.findFirst({
            where: and(eq(publicationCollaborationMembers.collaborationId, collaborationId), eq(publicationCollaborationMembers.userId, requesterId))
        });
        ...
        const existingMember = await db.query.publicationCollaborationMembers.findFirst({
            where: and(eq(publicationCollaborationMembers.collaborationId, collaborationId), eq(publicationCollaborationMembers.userId, data.userId))
        });
        if (existingMember) {
    ...
    static async searchUsersForInvite(query, limit = 10, excludeUserId) {
        ...
        const conditions = [
            ...
            or(ilike(users.username, term), ilike(users.email, term), ilike(users.firstName, term), ilike(users.lastName, term)),
        ];
    ...
    static async inviteCollaborator(collaborationId, requesterId, data) {
        ...
        if (data.userId != null) {
            ...
            const [u] = await db.select({ id: users.id }).from(users)
                .where(or(sql `lower(${users.username}) = ${identifier}`, sql `lower(${users.email}) = ${identifier}`))
                .limit(1);
            if (!u)
        ...
        const [existingRow] = await db.select().from(publicationCollaborationMembers)
            .where(and(eq(publicationCollaborationMembers.collaborationId, collaborationId), eq(publicationCollaborationMembers.userId, targetUserId)))
            .limit(1);
        if (existingRow) {
    ...
    static async respondInvite(collaborationId, inviteId, userId, data) {
        const invite = await db.query.publicationCollaborationInvites.findFirst({
            where: and(eq(publicationCollaborationInvites.id, inviteId), eq(publicationCollaborationInvites.collaborationId, collaborationId))
        });
        if (!invite)
    ...
    static async removeCollaborationMember(collaborationId, requesterId, memberId) {
        ...
        const targetMembership = await db.query.publicationCollaborationMembers.findFirst({
            where: and(eq(publicationCollaborationMembers.collaborationId, collaborationId), eq(publicationCollaborationMembers.userId, memberId))
        });
        if (!targetMembership) {
    ...
    static async getArticleDiscussions(articleId, userId) {
        const article = await db.query.publicationArticles.findFirst({
    ...
    static async postArticleDiscussion(articleId, userId, content, parentId) {
        const article = await db.query.publicationArticles.findFirst({
            where: eq(publicationArticles.id, articleId),
        });
        if (!article)
