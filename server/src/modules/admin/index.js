...
const createCrudHandlers = (path, table) => {
    ...
    admin.post(path, async (c) => {
        try {
            ...
            if (body.order === undefined && 'order' in getTableColumns(table))
                body.order = 0;
...
admin.get('/stats', async (c) => {
    try {
        const totalSantri = await db.select({ count: sql `count(*)` }).from(santri).then(res => res[0].count);
        const pendingPayments = await db.select({ count: sql `count(*)` }).from(payments).where(eq(payments.status, 'pending')).then(res => res[0].count);
        const verifiedPayments = await db.select({ count: sql `count(*)` }).from(payments).where(eq(payments.status, 'verified')).then(res => res[0].count);
        const acceptedSantri = await db.select({ count: sql `count(*)` }).from(santri).where(eq(santri.status, 'accepted')).then(res => res[0].count);
        const totalUsers = await db.select({ count: sql `count(*)` }).from(users).then(res => res[0].count);
        const maleCount = await db.select({ count: sql `count(*)` })
            .from(santri)
            .where(eq(santri.jenisKelamin, 'L'))
            .then(res => res[0].count);
        const femaleCount = await db.select({ count: sql `count(*)` })
            .from(santri)
...
admin.get('/users', zValidator('query', searchFilterSchema), async (c) => {
    ...
    if (query.search) {
        whereClause = or(like(users.username, `%${query.search}%`), like(users.email, `%${query.search}%`), like(users.firstName, `%${query.search}%`), like(users.lastName, `%${query.search}%`));
    }
...
admin.get('/users/:id', async (c) => {
    ...
    if (user.role === 'author' || user.publicationRole === 'author') {
        const [articles] = await db.select({ count: sql `count(*)` }).from(publicationArticles)
            .where(and(eq(publicationArticles.authorId, id), eq(publicationArticles.type, 'article')));
        const [journals] = await db.select({ count: sql `count(*)` }).from(publicationArticles)
            .where(and(eq(publicationArticles.authorId, id), eq(publicationArticles.type, 'journal')));
        const [approved] = await db.select({ count: sql `count(*)` }).from(publicationArticles)
            .where(and(eq(publicationArticles.authorId, id), eq(publicationArticles.status, 'approved')));
        const [pending] = await db.select({ count: sql `count(*)` }).from(publicationArticles)
...
admin.get('/santri', zValidator('query', searchFilterSchema), async (c) => {
    ...
    let whereClause = undefined;
    if (query.search) {
        whereClause = or(like(santri.namaLengkap, `%${query.search}%`), like(santri.nisn, `%${query.search}%`), like(santri.noHp, `%${query.search}%`), like(santri.email, `%${query.search}%`));
    }
    if (query.status) {
...
admin.get('/payments', zValidator('query', searchFilterSchema), async (c) => {
    ...
    if (query.status) {
        if (query.status !== 'accepted') {
            whereClause = eq(payments.status, query.status);
        }
    }
