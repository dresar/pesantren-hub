...
blog.get('/posts', zValidator('query', blogFilterSchema), async (c) => {
    try {
        ...
        if (query.search) {
            whereClause = or(like(posts.title, `%${query.search}%`), like(posts.content, `%${query.search}%`), like(posts.excerpt, `%${query.search}%`));
        }
        if (query.status) {
...
blog.post('/posts', adminMiddleware, zValidator('json', createPostSchema), async (c) => {
    ...
    if (data.tags && data.tags.length > 0) {
        await db.insert(postTags).values(data.tags.map(tagId => ({
