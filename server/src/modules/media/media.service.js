...
export class MediaService {
    ...
    async listAccounts(query = {}) {
        let conditions = [];
        if (query.provider)
            conditions.push(eq(mediaAccounts.provider, query.provider));
        if (query.isActive !== undefined)
            conditions.push(eq(mediaAccounts.isActive, query.isActive));
        ...
        try {
            if (account.provider === 'cloudinary' && cloudName) {
                ...
                const result = await db.select({
                    totalSize: sql `sum(${mediaFiles.size})`
    ...
    async listFiles(query) {
        ...
        let conditions = [];
        if (query.category)
            conditions.push(eq(mediaFiles.category, query.category));
        if (query.folder) {
        ...
        if (query.type) {
            if (query.type === 'image') {
                ...
                conditions.push(or(like(mediaFiles.mimeType, 'application/pdf'), like(mediaFiles.mimeType, 'application/msword'), like(mediaFiles.mimeType, 'application/vnd.openxmlformats-officedocument.wordprocessingml.document')));
            }
        }
