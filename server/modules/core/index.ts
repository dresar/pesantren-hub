...
core.get('/health/db', async (c) => {
  try {
    ...
    return c.json({ ok: res.rows[0]?.ok === 1 });
  } catch (e: any) {
...
core.post('/contact', zValidator('json', createKontakSchema), async (c) => {
  const data = c.req.valid('json');
  const [result] = await db.insert(kontak).values({
    ...data,
    status: 'pending',
    balasan: '',
    createdAt: new Date(),
  ...
  const [item] = await db.select().from(kontak).where(eq(kontak.id, result.insertId));
  return c.json(item);
});
...
core.get('/hero', async (c) => {
  try {
    ...
    if (data.length === 0) {
      const [result] = await db.insert(heroSection).values({
      ...
      const [item] = await db.select().from(heroSection).where(eq(heroSection.id, result.insertId));
      return c.json([item]);
    }
    return c.json(data);
  } catch (e) {
...
core.post('/whatsapp-templates', adminMiddleware, zValidator('json', createWhatsAppTemplateSchema), async (c) => {
  ...
  const [item] = await db.select().from(whatsappTemplates).where(eq(whatsappTemplates.id, result.insertId));
  return c.json(item);
});
...
core.post('/tenaga-pengajar', adminMiddleware, zValidator('json', createTenagaPengajarSchema), async (c) => {
  const data = c.req.valid('json');
  const [result] = await db.insert(tenagaPengajar).values(data as any);
  const [item] = await db.select().from(tenagaPengajar).where(eq(tenagaPengajar.id, result.insertId));
  return c.json(item);
});
...
const createSimpleCrud = (path: string, table: any, createSchema: any, updateSchema: any, orderByField: any = null) => {
  ...
  core.post(`/${path}`, adminMiddleware, zValidator('json', createSchema), async (c) => {
    ...
    const [item] = await db.select().from(table).where(eq(table.id, result.insertId));
    return c.json(item);
  });
...
core.post('/sejarah-timeline', adminMiddleware, zValidator('json', createSejarahTimelineSchema), async (c) => {
  ...
  const [result] = await db.insert(sejarahTimeline).values({
    ...timelineData,
